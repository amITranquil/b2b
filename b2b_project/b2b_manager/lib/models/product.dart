class Product {
  ...
  final bool isManual; // true = Manual product, false = API product
  ...
  // Satış fiyatları (KDV hariç ve dahil)
  double get salePriceExcludingVat => vatRate > 0 ? myPrice / (1 + vatRate / 100) : myPrice;
  double get salePriceIncludingVat => myPrice;

  // Quote sistemine geçiş için gerekli alanlar
  String get stockCode => productCode;
  double get specialPrice => buyPriceIncludingVat;
  double get discountRate => discount1;
  double get calculatedPrice => salePriceExcludingVat; // KDV hariç fiyat
  double get calculatedPriceWithVat => myPrice;
  int get discountType => 0;
  double quantity = 0; // Quote için kullanılacak
  double customDiscount = 0; // Quote için kullanılacak
  double profitMargin = 0; // Quote için kullanılacak
